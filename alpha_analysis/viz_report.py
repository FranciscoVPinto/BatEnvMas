...
def plot_overview(axs, t: pd.Index, community_pv: pd.Series, load_df: pd.DataFrame,
    ...
    ax = axs[0,0]
    ax.plot(t, community_pv, label="PV", color="#f59e0b")
    ax.plot(t, load_df.sum(axis=1), label="Carga total", color="#2563eb")
    ...
    ax = axs[0,1]
    alloc_tot = first_alloc.sum(axis=1)
    import_series = (load_df.sum(axis=1) - alloc_tot).clip(lower=0)
    export_series = (community_pv - alloc_tot).clip(lower=0)
    ...
    # 3) Curvas de duração (Import/Export)
    ax = axs[1,0]
...
def plot_alpha_heatmaps(out_dir: Path, t: pd.Index, alpha_by_cfg: Dict[str, pd.DataFrame],
    ...
    for name, A in alpha_by_cfg.items():
        ...
        if receiver_mask is not None:
            data = data.where(receiver_mask, other=0.0)
...
def plot_per_agent_panels(out_dir: Path, t: pd.Index, agent: str,
    ...
    axs[0,0].plot(t, load, label="Load", color="#1f77b4")
    axs[0,0].plot(t, pv, label="PV total", color="#ff7f0e", alpha=0.6)
    ...
    for name, A in alloc_by_cfg.items():
        ...
        axs[1,0].plot(t, net_import, label=name)
