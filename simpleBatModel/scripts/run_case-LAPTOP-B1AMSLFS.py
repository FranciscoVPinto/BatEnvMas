...
def _solver_available_local(name: str) -> bool:
    """Pre-check executables/modules to avoid noisy Pyomo warnings."""
    ...
    except Exception:  # SolverFactory may raise for unknown names
        return False
...
def _configure_highs_logging(opt) -> None:
    """Stream solver output and write `highs.log` when supported."""
...
def _solve_model_safe(model, *, solver: str, options: Optional[dict], tee: bool):
    """
    Try the preferred solver, then any fallback that is actually available.

    For APPSI solvers we set `config.load_solutions=False` and load the solution
    manually only when the termination condition is feasible/optimal — this
    avoids the well-known crash when the solver finds the model infeasible.
    ...
    for s in chain:
        try:
            ...
            return results, s
        except Exception as e:  # noqa: BLE001  (intentional: try next solver)
...
def _diagnose_multi_house_power(
    *,
    ...
    """
    Sufficient-condition checks (per-house and system-level):
      Load[h,t] > PV[h,t] + P_grid_max[h] + P_dis_max[h]   => house infeasible
      sum_h Load[h,t] > sum_h (PV[h,t] + P_grid_max[h] + P_dis_max[h])  => system infeasible
    """
...
def _handle_infeasible(
    *,
    ...
    """Build a diagnostic report and return a RuntimeError for the caller to raise."""
...
def run_case(
    ...
    *,
    ...
    """
    Run one case YAML end-to-end and write results into `outputs_dir/<case_name>/`.

    Pipeline: load+canonicalize cfg → infer horizon → load loads/PV/tariffs →
    build & solve MultiHouseModel → write per-house CSVs and meta.yaml.
    On infeasibility, writes `debug_infeasibility.txt` and raises RuntimeError.
    """
    # ----- configuration -----
    ...
    # ----- inputs (loads, PV, tariffs, battery params) -----
    ...
    except Exception as e:  # noqa: BLE001  (any solve failure → diagnose + re-raise)
