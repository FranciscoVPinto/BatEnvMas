...
def _per_house_pv(
    ...
    if isinstance(pv_cfg, dict):
        ...
        return {
            h: load_series_csv_1col(_abs_from_root(root, pv_cfg_str[h]), T=T)
            for h in houses
        }
...
def _alphas_from_scalar(
    ...
    *,
    ...
    """
    Build constant-in-time alpha series from sharing.alpha = {house: scalar}.
    Calls `fallback_or_raise(reason, raise_msg)` when validation fails:
      - if a fallback path is configured, it returns the alpha series built from loads;
      - otherwise, it raises ValueError(raise_msg).
    """
...
def _alphas_from_profile(
    ...
    *,
    ...
    """
    Build per-timestep alpha series from sharing.alpha_profile = {house: csv_path}.
    Validation order (matches legacy behavior): negativity per-(h, t) is checked
    while loading; strict_sum_to_one is checked at the first violating t.

    Returns:
      alpha_series: {house: [alpha(t)]}, post-normalization
      alpha_sum_pre_norm: pre-normalization sums per t (for debug["alpha_sum"]),
                         or None when the result came from a fallback path.
    """
...
def _finalize_shared(
    ...
    *,
    ...
    """
    Multiply alpha_series by pv_total to get pv_by_house and populate debug.

    `alpha_sum` lets the caller pass pre-normalization sums (used by alpha_profile
    paths so debug["alpha_sum"] stays diagnostic). When None, sums are computed
    from `alpha_series` directly.
    """
    if alpha_sum is None:
        alpha_sum = [sum(alpha_series[h][t] for h in houses) for t in range(T)]
    pv_by_house = {
        h: [alpha_series[h][t] * pv_total[t] for t in range(T)] for h in houses
    }
    debug.update({
...
def prepare_pv_by_house(
    ...
    *,
    ...
    # ----- Mode A: per-house PV (no sharing) -----
    if has_pv:
        info["mode"] = "per_house"
    ...
    # Build the fallback alpha series silently (no warning). Used by B1/B2,
    # which choose the fallback path explicitly via configuration.
    ...
    # Used by B3/B4 when validating provided alphas. Adds a warning, then either
    # falls back (if fb_mode is supported) or raises with the caller's specific
    # validation message — preserving legacy behavior where an unsupported
    # fb_mode lets the original validation error surface.
    ...
    # B3 / B4: validated alpha_map or alpha_profile (with optional fallback on failure)
    if alpha_map is not None:
        alpha_series = _alphas_from_scalar(
            ...
            strict_sum_to_one=strict_sum_to_one,
            info=info,
            fallback_or_raise=fallback_or_raise,
        )
        return _finalize_shared(alpha_series, pv_total, houses, T, info, debug)
